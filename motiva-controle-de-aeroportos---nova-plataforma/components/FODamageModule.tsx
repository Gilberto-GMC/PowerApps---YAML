...
interface FODamageRecord {
  ...
  location: string;
  position: string;
  source: string; // Origem provável do detrito causador
  description: string;
}

const MOCK_RECORDS: FODamageRecord[] = [
  {
    id: '1',
    ...
    source: 'Aeronave',
    description: 'Dano na carenagem do motor esquerdo causado por impacto de parafuso metálico solto na pista.'
  },
  {
    id: '2',
    ...
    date: '2024-05-17',
    time: '16:20',
    materialType: 'Borracha (Pneu)',
    location: 'Pátio de Estacionamento',
    position: 'Posição 05',
    source: 'Veículo / Equipamento de Apoio',
    description: 'Pneu do veículo de apoio avariado por fragmento de borracha dura no pátio.'
  }
];
...
export const FODamageModule: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'form' | 'history'>('form');
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [records, setRecords] = useState<FODamageRecord[]>(MOCK_RECORDS);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      const newRecord: FODamageRecord = {
  ...
  const handleEdit = (record: FODamageRecord) => {
    setFormData({
    ...
    <div className="flex flex-col gap-[26px] relative h-full w-full">
      ...
      <div className="flex p-1 bg-slate-50 rounded-none w-fit border border-slate-200 shrink-0">
        ...
        >
          ...
          {editingId ? 'Editar Dano' : 'Novo Registro'}
        </button>
        <button
          ...
          <div className="max-w-5xl animate-in fade-in duration-500 pb-10">
            {isSubmitting ? (
              <div className="flex flex-col items-center justify-center py-24 text-center animate-in zoom-in duration-300">
                ...
                <h3 className="text-2xl font-bold text-slate-900 mb-2">{editingId ? 'Atualizando...' : 'Registrando Dano...'}</h3>
              ...
              <form onSubmit={handleSubmit} className="space-y-[26px]">
                <div className="bg-amber-50 border border-amber-200 p-6">
                  <div>
                    ...
                    <p className="text-[13px] text-amber-800 font-medium leading-relaxed">
                      Este módulo deve ser utilizado exclusivamente para registrar danos causados por detritos (FOD) encontrados em aeronaves ou veículos/equipamentos terrestres nas áreas operacionais. Caso queira reportar apenas o achado de um detrito sem dano físico associado, use o módulo Detritos (F.O.Debris).
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-[26px]">
                  <div className="space-y-2">
                    <label className="text-[13px] font-bold text-slate-900 block">Tipo de Material Causador *</label>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-[26px]">
                  <div className="space-y-2">
                    <label className="text-[13px] font-bold text-slate-900 block">Local do Dano / Achado *</label>
                  ...
                  <div className="space-y-2">
                    ...
                    <input required type="text" placeholder="Ex: Carenagem do motor esquerdo, pneu dianteiro do pushback..." className="w-full px-5 py-4 bg-white border border-slate-200 rounded-none text-[13px] text-slate-700 outline-none focus:border-[#c026d3] transition-all font-medium" value={formData.position} onChange={(e) => setFormData({...formData, position: e.target.value})} />
                  </div>
                </div>

                <div className="space-y-2">
                  <label className="text-[13px] font-bold text-slate-900 block">Descrição Detalhada do Dano e Detrito *</label>
                  <textarea required rows={4} placeholder="Descreva os danos observados no equipamento/aeronave e os detalhes do objeto estranho causador..." className="w-full px-5 py-4 bg-white border border-slate-200 rounded-none text-[13px] text-slate-700 outline-none focus:border-[#c026d3] transition-all font-medium resize-none" value={formData.description} onChange={(e) => setFormData({...formData, description: e.target.value})} />
                </div>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white rounded-none w-full max-w-sm shadow-2xl border border-slate-100 overflow-hidden">
            <div className="p-8 text-center">
              ...
              <p className="text-slate-500 text-sm leading-relaxed mb-6">Esta ação não poderá ser desfeita. O registro de dano será removido do histórico.</p>
            </div>
