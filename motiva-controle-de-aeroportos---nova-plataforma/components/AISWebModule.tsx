...
export const AISWebModule: React.FC = () => {
      ...
      <div className="flex-1 flex flex-col overflow-hidden animate-in fade-in duration-500 bg-white" onClick={() => setActionMenuId(null)}>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="max-w-full mx-auto space-y-6">
            <div className="flex items-center gap-4 bg-slate-50 p-4 rounded-none border border-slate-100 shadow-inner">
              <div className="flex items-center gap-2">
                <Filter className="w-4 h-4 text-slate-400" />
                <span className="text-xs font-black text-slate-400 uppercase tracking-widest">Filtrar Aeródromo:</span>
              </div>
              <div className="relative">
                <select 
                  value={historyFilter}
                  onChange={(e) => setHistoryFilter(e.target.value)}
                  className="h-10 pl-4 pr-10 bg-white border border-slate-200 rounded-none text-xs font-bold text-slate-700 outline-none focus:border-brand-500 transition-all appearance-none cursor-pointer"
                >
              ...
              <table className="w-full text-center border-collapse table-fixed">
                <thead>
                  <tr className="bg-slate-50/50 border-b border-slate-100">
                    <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-wider text-center">Data</th>
                    <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-wider text-center">Aeroporto</th>
                    <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-wider text-center">Conformes</th>
                    <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-wider text-center">Não Conformes</th>
                  ...
                  {historyRecords.map((record) => {
                    ...
                    return (
                      <tr key={record.id} className="hover:bg-slate-50/30 transition-colors">
                        <td className="px-6 py-4 text-xs font-semibold text-slate-900 text-center">{record.date.split('-').reverse().join('/')}</td>
                        <td className="px-6 py-4 text-xs font-bold text-slate-800 uppercase text-center">{record.airport}</td>
 ...
 <div className="absolute z-50 right-1/2 translate-x-1/2 mt-1 w-32 bg-white border border-slate-200 rounded-sm shadow-xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
 <button 
 ...
 <Pencil className="w-3.5 h-3.5" /> Editar
      ...
      <div className="flex-1 flex flex-col overflow-y-auto bg-slate-50/50 p-6 animate-in fade-in duration-500 pb-10">
        <div className="max-w-5xl mx-auto w-full space-y-8 pb-10">
          ...
          <div className="bg-white p-8 rounded-none border border-slate-100 shadow-sm">
            ...
            {!isReadOnlyMode ? (
              <button 
                disabled={!isAuditComplete || (failedItems.length > 0 && !areSdiaCodesFilled)}
                onClick={() => setIsFinalizing(true)}
                className="w-full flex items-center justify-center gap-2 py-4 bg-brand-500 hover:bg-brand-600 text-white rounded-none text-sm font-bold transition-all shadow-lg shadow-indigo-900/20 disabled:opacity-50 uppercase"
              >
  ...
  const renderMainContent = () => {
    ...
    if (!selectedAirport) {
      return (
        <div className="flex-1 flex flex-col items-center justify-center p-12 text-center h-full bg-white animate-in fade-in duration-500">
          <div className="max-w-md w-full">
            ...
            <div className="relative">
              <select 
                ...
                className="w-full h-16 pl-6 pr-12 bg-white border-2 border-slate-200 rounded-none text-base font-bold text-slate-700 outline-none focus:border-brand-500 transition-all appearance-none cursor-pointer"
              >
      ...
      <div className="flex-1 flex flex-col overflow-hidden bg-white">
        <div className="flex-1 overflow-y-auto p-12">
          <div className="max-w-5xl mx-auto space-y-6">
            <div className="mb-10">
              <h2 className="text-[26px] font-bold text-slate-900 mb-2 leading-tight">{currentSection?.title}</h2>
              <p className="text-lg text-slate-500 leading-relaxed font-normal">{currentSection?.description}</p>
            </div>
                  ...
                  <div className="px-8 py-6 flex flex-col gap-4">
                    <div className="flex items-center justify-between gap-8">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-slate-400 font-normal leading-relaxed mt-1">{item.description}</p>
        ...
        <div className="bg-white border-t border-slate-100 p-8 flex justify-end gap-3 shrink-0">
          <button 
            onClick={() => { if (activeSectionId < 14) setActiveSectionId(activeSectionId + 1); else setShowSummary(true); }} 
            className="px-12 py-3 bg-brand-500 hover:bg-brand-600 text-white font-bold rounded-none text-sm uppercase tracking-wider transition-all"
          >
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-300">
           <div className="bg-white rounded-none w-full max-w-sm shadow-2xl border border-slate-100">
              <div className="p-10 text-center">
                ...
                <h3 className="text-xl font-black text-slate-900 mb-2 tracking-tight uppercase">Audit Concluída</h3>
                <p className="text-sm text-slate-500 mb-8">O relatório de verificação AISWEB foi salvo com sucesso.</p>
                <button onClick={() => { setIsFinalizing(false); setViewMode('history'); setSelectedAirport(''); }} className="w-full py-3 bg-brand-500 rounded-none text-xs font-black text-white uppercase tracking-widest shadow-lg shadow-indigo-900/20">OK</button>
              </div>
           </div>
        </div>
      )}
