...
interface GroundOccurrenceRecord {
  id: string;
  airport: string;
  date: string;
  time: string;
...
const MOCK_RECORDS: GroundOccurrenceRecord[] = [
  {
    id: '1',
    ...
    partStruck: 'Fuselagem',
    damageAndLoss: 'Sim',
    location: 'Pátio de Estacionamento',
    position: 'Posição 04',
    description: 'Veículo de apoio (QTA) encostou na fuselagem traseira durante aproximação para serviço.',
    hasInjuries: 'Não',
    injuredPeople: [],
    status: 'Em Análise'
  },
  {
    ...
    date: '2024-05-16',
    time: '15:40',
    aircraftRegistration: 'PS-ADU',
    aircraftModel: 'A320',
    aircraftOperator: 'Azul Linhas Aéreas',
    partStruck: 'Não aplicável',
    damageAndLoss: 'Não',
    location: 'Pátio de Estacionamento',
    position: 'Posição 02',
    description: 'Queda de container de bagagem durante descarregamento por falha na trava do dolly.',
    hasInjuries: 'Não',
    injuredPeople: [],
    status: 'Finalizado'
  }
...
export const GroundOccurrenceModule: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'form' | 'history'>('form');
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [records, setRecords] = useState<GroundOccurrenceRecord[]>(MOCK_RECORDS);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      const newRecord: GroundOccurrenceRecord = {
  ...
  const handleEdit = (record: GroundOccurrenceRecord) => {
    setFormData({
          ...
          <div className="max-w-5xl animate-in fade-in duration-500 pb-10">
            {isSubmitting ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-[26px]">
                {/* Definição CENIPA */}
                <div className="bg-blue-50 border border-blue-200 p-6 flex gap-4 shadow-sm">
                  <AlertOctagon className="w-6 h-6 text-blue-600 shrink-0" />
                  <div>
                    <h4 className="text-[13px] font-bold text-blue-900 uppercase tracking-wide mb-2">Definição Operacional (CENIPA)</h4>
                ...
                <div className="border border-slate-100 p-6 bg-slate-50/30">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-[26px] mt-[26px]">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        <select required className="w-full px-5 py-4 bg-white border border-slate-200 rounded-none text-[13px] text-slate-700 outline-none focus:border-[#c026d3] transition-all appearance-none cursor-pointer font-medium" value={formData.damageAndLoss} onChange={(e) => setFormData({...formData, damageAndLoss: e.target.value})}>
                          <option value="">Selecione...</option>
                      ...
                      {formData.injuredPeople.map((person, index) => (
                        <div key={person.id} className="flex flex-col md:flex-row gap-4 items-start md:items-end bg-white p-4 border border-slate-200 relative">
                          <div className="w-full md:w-1/3 space-y-2">
                            ...
                            <input required type="text" placeholder="Ex: João Silva (Mecânico)" className="w-full px-4 py-3 bg-white border border-slate-200 rounded-none text-[13px] text-slate-700 outline-none focus:border-[#c026d3] transition-all font-medium" value={person.name} onChange={(e) => {
 ...
 <div className="text-[10px] text-slate-500 font-medium">{record.time} UTC</div>
                      ...
                      <td className="px-6 py-4 text-center">
 <span className="text-xs font-bold text-slate-700 uppercase">{record.aircraftRegistration}</span>
 </td>
                      <td className="px-6 py-4 text-center">
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white rounded-none w-full max-w-sm shadow-2xl border border-slate-100 overflow-hidden">
            <div className="p-8 text-center">
              ...
              <p className="text-slate-500 text-sm leading-relaxed mb-6">Esta ação removerá permanentemente o registro de ocorrência do banco de dados.</p>
            </div>
